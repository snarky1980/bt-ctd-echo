  ...
  const copyAdminStaticPlugin = {
    ...
    writeBundle(options) {
      ...
      const files = [
        ...
        { src: path.resolve(root, 'CNAME'), dst: path.resolve(outDir, 'CNAME') },
        { src: path.resolve(root, 'complete_email_templates.json'), dst: path.resolve(outDir, 'complete_email_templates.json') },
      ];
      const assets = [
        ...
        { src: path.resolve(root, 'assets', 'rich-text-toolbar.js'), dst: path.resolve(outDir, 'assets', 'rich-text-toolbar.js') },
          ...
          if (isHtml && path.basename(f.src) === 'admin-simple.html') {
            try {
              ...
              html = html.replace(
                ...
                /(<script\s+src=["']\.\/assets\/rich-text-toolbar\.js)(["'])/,
                `$1?v=${stamp}$2`
              );
